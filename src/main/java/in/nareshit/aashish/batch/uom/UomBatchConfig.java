...
public class UomBatchConfig {
	...
	public ItemReader<Uom> reader() {
		...
		reader.setResource(new FileSystemResource("C:\\Users\\aashish\\Desktop\\HK\\whapp\\uoms.csv"));
	...
	public ItemWriter<Uom> writer() {
		...
		JpaItemWriter<Uom> writer = new JpaItemWriter<Uom>();
	...
	@Bean
	public Step stepUom() {
		return sf.get("stepUom").<Uom, Uom>chunk(10).reader(reader()).processor(processor()).writer(writer()).build();
	}
	...
	@Bean
	public Job jobUom() {
		return jf.get("jobUom").listener(new UomListener()).incrementer(new RunIdIncrementer()).start(stepUom())
				.build();
	}
